...
<form action="index.php" method="post" name="adminForm" id="adminForm">
    <div class="width-60 ">
        <fieldset class="adminform">
            ...
            <ul class="adminformlist">
					...
					<span id="vm_cat_box">
                        <select name="vm_cat[]">
                            ...
                            foreach ($this->vm_categories as $vm_cat) {
                                ...
                                $selected = "";
                                if ($vm_cat->category_id == $this->vm_cat) {
                ...
                <li>
                    <label for="custom_fields">Połącz:</label>
                    <span id="custom_fields">
					<?php
                    echo "<table id='fields_and_attribs' border='0' style='float: left; width: auto; margin: 5px 5px 5px 0;'>";
                    foreach ($this->ceneo_category_attribs as $k => $ceneo_category_attrib) {
                        echo "<tr><td>" . $ceneo_category_attrib->name . "</td><td>";
                        echo "<select name='customF[]'>";
                        echo "<option value=''>-- puste --</option>";

                        $selected_mf = "";
                        if (in_array($ceneo_category_attrib->id . "_mfname", $this->selected_custom_fields_codes)) {
                            $selected_mf = " selected='selected' ";
                        }

                        $selected_sku = "";
                        if (in_array($ceneo_category_attrib->id . "_sku", $this->selected_custom_fields_codes)) {
                            $selected_sku = " selected='selected' ";
                        }

                        echo "
						<optgroup label='Producent'>
							<option " . $selected_mf . " value='" . $ceneo_category_attrib->id . "_mfname'>Nazwa producenta</option>
						</optgroup>
						<optgroup label='Produkt'>
							<option " . $selected_sku . " value='" . $ceneo_category_attrib->id . "_sku'>SKU produktu</option>
						</optgroup>
						<optgroup label='Pola dodatkowe'>";
                        foreach ($this->custom_fields as $field) {
                            $selected = "";
                            if (in_array($ceneo_category_attrib->id . "_" . $field->virtuemart_custom_id,
                                $this->selected_custom_fields_codes)) {
                                $selected = " selected='selected' ";
                            }
                            echo "<option " . $selected . " value='" . $ceneo_category_attrib->id . "_" . $field->virtuemart_custom_id . "'>" . $field->custom_title . "</option>";
                        }
                        echo "</optgroup></select></td>";
                        $default = ($ceneo_category_attrib->is_default ? 'atrybut domyślny' : 'atrybut własny &nbsp;&nbsp;&nbsp;<a href="JavaScript:void(0);" onclick="deleteAttribute(' . $ceneo_category_attrib->id . ', this);">Usuń atrybut</a>');
                ...
                <li>
                    <label><b>Ogranicz do produktów:</b></label>
                </li>
                <li>
                    <label>Ogranicz produkty do ceneo - wybierz tylko te produkty z kategorii,</br> które chcesz
                        umieścić w pliku Ceneo (domyślnie wszystkie):</label>
                    <select name="ceneo_zakres_produktow[]" multiple="multiple" size="10">
                        <?php

                        if (!empty($this->vm_category_products)) {
                            foreach ($this->vm_category_products as $product) {
                                echo "<option value='" . $product->virtuemart_product_id . "' " . (in_array($product->virtuemart_product_id,
                                        $this->vm_category_products_selected) ? " selected='selected' " : "") . ">" . $product->product_name . "</option>";
                            }
                        } else {
                            echo "<option value=''>Brak produktów w tej kategorii</option>";
                        }
    ...
    <input type="hidden" name="xref_id" value="<?php echo $this->xref_id; ?>"/>
...
$$('#additional_fields button').addEvent('click', function (event) {
        ...
        if (!$('nowy_atrybut')) {
            ...
            //nowy_atrybut.innerHTML = select_input.innerHTML + '<div>Podaj nazwę atrybutu: </div>';
